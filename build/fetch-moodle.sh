...
#
#  Sources are tried in order until one yields a valid gzip tarball:
...
try_git() {
    ...
    say "Trying git clone of tag v${version}"
    git clone --depth 1 --branch "v${version}" --quiet \
        https://github.com/moodle/moodle.git "$tmp/moodle" || { fail "git clone failed"; return 1; }
...
say "Installed Moodle ${release} (source: ${got})"
