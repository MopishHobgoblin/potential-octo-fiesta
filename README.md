...
|---|---|
| Distribution | **MuTMS 5.2.2** by default (Moodle 5.2.2 + multi-tenancy patch and the MuTMS plugin suite). Set `MOODLE_DISTRIBUTION=moodle` for plain Moodle |
...
| `MOODLE_ALLOW_WEB_PLUGIN_INSTALL` | `false` | Allow installing plugins from the web UI (see Plugins) |
| `MOODLE_PLUGIN_SYNC_INTERVAL` | `300` | Seconds between plugin saves when the above is on |
...
## Distribution: MuTMS or plain Moodle

The image builds [MuTMS](https://github.com/mutms/mutms) by default — a GPL-3.0 distribution that is standard Moodle plus a small multi-tenancy core patch and the MuTMS plugin suite: multi-tenancy, programs, certifications, training credits, supervisors and teams, custom home pages, interactive book, breached-password blocking, privileged sessions and log-in-as via Incognito.

```bash
MOODLE_DISTRIBUTION=mutms     # default
MOODLE_DISTRIBUTION=moodle    # plain upstream Moodle
```

MuTMS releases track Moodle point releases one for one, so `MOODLE_VERSION` still drives everything: `5.2.2` installs the `MuTMS-5.2.2-01` tag, built on Moodle 5.2.2. Pin an exact tag with `MUTMS_TAG=MuTMS-5.2.2-02` when a second revision of the same Moodle release is published. Tags are listed at <https://github.com/mutms/mutms/releases>.

Worth knowing:

- MuTMS is an independent project, not affiliated with or endorsed by Moodle Pty Ltd, and it isn't listed in the Moodle Marketplace — Moodle classes it as competing with Moodle Workplace. Plugin updates therefore come from the image, not from inside Moodle.
- Only 5.1 and 5.2 releases work here. MuTMS also publishes 4.5.x and 5.0.x, but those predate Moodle's `public/` directory, which this image's nginx config depends on. The build stops with a clear message if you pick one.
- MuTMS packages carry no published checksum, so the build verifies TLS and that the archive is a valid gzip, rather than a SHA-256. The plain Moodle path still verifies its SHA-256.
- The plugins arrive as part of the code tree, so they're treated as core and aren't copied into `/config/plugins`. Your own add-ons still are.
- Switching an existing site from `moodle` to `mutms` works — the plugins install on the next start. Going back is not a simple downgrade: uninstall the MuTMS plugins in Moodle first, or restore a backup.
- Multi-tenancy is off until configured. See <https://docs.mutms.org>.

## Plugins and themes

There are two ways to add plugins. Both survive image updates; pick one.

### 1. Folder-based (default)

Place plugins in `/config/plugins` using the same layout as Moodle's `public/` folder, then restart the container:
...
The folder must be named after the plugin's component — `mod/customcert`, not `mod/moodle-mod_customcert-v4.5`, which is how ZIPs from moodle.org usually unpack. On start they are copied into the code tree and `upgrade.php` installs them.

The whole code tree stays owned by root and read-only to the web server, which is what Moodle recommends for production.

### 2. From the Moodle web interface

Set `MOODLE_ALLOW_WEB_PLUGIN_INSTALL=true` and *Site administration → Plugins → Install plugins* works as on a normal Moodle server, including one-click updates for add-ons.

To make that safe with an image-based code tree, two things happen:

- Only the plugin **type** directories (`mod`, `blocks`, `theme`, `admin/tool`, `question/type`, subplugin directories, and so on) become writable by the web server. Moodle's own plugin folders stay root-owned, so the installer cannot overwrite core code.
- A background job (`moodle-plugin-sync`) copies anything you install into `/config/plugins` every 5 minutes and again when the container stops, so it is restored after an image update. Uninstalling a plugin in Moodle removes it from `/config/plugins` too, so it doesn't come back.

Plugins are identified by their `version.php` and compared against a manifest of core plugins recorded when the image was built.

```bash
docker exec moodle moodle-plugin-sync --once   # save immediately, don't wait
```

Trade-off: the web server can write to the plugin type directories, so a PHP vulnerability could plant code there. Moodle's own advice is to keep the code read-only, which is why this is off by default. Leave it off if you rarely add plugins, or turn it on while installing and off again afterwards — plugins already saved to `/config/plugins` keep working either way.

## Upgrading Moodle

1. Change `MOODLE_VERSION` and `MOODLE_BRANCH` together (on MuTMS, check the matching tag exists first) — the branch is the version without the dot, so 5.2.2 → `502`, 5.1.6 → `501`, 5.3.0 → `503`. Both are listed on <https://download.moodle.org/releases/>.
...
docker exec -it moodle moodle-plugin-sync --once   # save web-installed plugins now
