...
# moodle = standard Moodle. mutms = the MuTMS distribution (Moodle plus the
# multi-tenancy core patch and the MuTMS plugin suite), whose releases track
...
RUN MOODLE_VERSION="${MOODLE_VERSION}" \
    ...
    MOODLE_DISTRIBUTION="${MOODLE_DISTRIBUTION}" \
    MUTMS_TAG="${MUTMS_TAG}" \
...
RUN set -eux; \
    chmod 0755 /usr/local/bin/docker-entrypoint /usr/local/bin/moodle-cli \
               /usr/local/bin/moodle-cron /usr/local/bin/moodle-plugin-sync \
               /usr/local/bin/healthcheck; \
    for f in /usr/local/bin/docker-entrypoint /usr/local/bin/moodle-cli /usr/local/bin/moodle-cron \
             /usr/local/bin/moodle-plugin-sync /usr/local/bin/healthcheck \
             /usr/local/lib/moodle/common.sh; do bash -n "$f"; done; \
